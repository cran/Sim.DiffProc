...
%\VignetteIndexEntry{It\^{o} and Stratonovich Stochastic Calculus}
%\VignetteKeywords{stochastic calculus, Stratonovich calculus, It\^{o} calculus, stochastic differential equation, simulation}
%\VignettePackage{Sim.DiffProc}

\begin{document}

\title{\bfseries{It\^{o} and Stratonovich Stochastic Calculus with}\\
      \vskip3pt \large \CRANpkg{Sim.DiffProc} Package Version 2.6}
...
\end{figure}
If we simulate 100 trajectories and let the settings above unchanged (except for the number of simulations, of course); Using Monte-Carlo 
...
@
\end{center} 
\caption{$100$ trajectories of It\^{o} SDE \code{'mod1'} (Left), and Stratonovich SDE \code{'mod2'} (Right).}\label{fig2}
\end{figure}

Next is an example for an It\^{o} SDE that is used in experimental psychology to describe the coordinated movement of a subject's left and right index fingers,
$$
dX_{t} = -(a\sin(X_{t})+2b\sin(2X_{t}))dt+\sigma dW_{t},
$$
for this SDE, we simulate a flow of $100$ trajectories, with integration stepsize $\Delta t=0.002$, $a=0.5$, $b=1$ and $\sigma=0.1$, and 
using stochastic Runge-Kutta methods $3$-stage, we get the plots in Figure \ref{fig0}, 
<<>>=
a = 0.5; b=1; sigma=0.1
...
@ 
\end{center}
\caption{Flow paths for an It\^{o} SDE that is used in experimental psychology.}\label{fig0}
\end{figure}
...
simulate $100$ trajectories to radial process \eqref{eq08} by \code{snssde1d} function, and the graphical representation can be seen in Figure \ref{fig3},
<<>>=
K = 4; s = 1; sigma = 0.2
...
\right)
\end{equation}
We illustrate the usage of the \code{snssde2d} function to simulate the solution of a  It\^{o} \eqref{eq09} or Stratonovich \eqref{eq10} SDE's two dimensional, by two applications.
...
Next is an example of a system of Stratonovich SDE's $2$-dimensional which describing autonomous oscillations.
...
mod2d
@
which can be plotted in the plane $(O,X,Y)$ using the command \code{plot2d}, and the result is shown in Figure \ref{fig4} and \ref{fig5}:
<<print=FALSE,eval=FALSE>>=
plot2d(mod2d,type="l")
plot(mod2d)
@
...
@
\end{center} 
\caption{$2D$ stochastic Van-der-Pol equation (Left). Relaxation oscillation in the Van der Pol oscillator (Right) ($\mu=4$ and $\sigma=0.1$).}\label{fig4}
\end{figure}
\setkeys{Gin}{width=0.45\textwidth}
\begin{figure}[!ht]
\begin{center}
...
@
\end{center} 
\caption{$2D$ stochastic Van-der-Pol equation (Left). Relaxation oscillation in the Van der Pol oscillator (Right) ($\mu=0.2$ and $\sigma=0.1$).}\label{fig5}
\end{figure}

\subsection{The \texttt{snssde3d()} function}
A system of three SDE's for the triple $(X_t, Y_t ,Z_t)$ driven by three independent Brownian motions
...
% \nonumber to remove numbering (before each equation)
  ...
  dZ_t &=&  dW_{2,t} \nonumber
\end{eqnarray}
run by calling the function \code{"snssde3d"} to produce a simulation of the solution of \eqref{eq20}:
<<>>=
mu = 2; sigma=0.2
fx <- expression(mu*y)
...
@
\end{center} 
\caption{Simulated path of: $dX_{t} = 2 W_{1,t} dt + 0.2 W_{2,t} dW_{3,t}$ used \code{snssde3d} function.}\label{fig7}
\end{figure}
...
Guidoum, A. C. and Boukhetala, K. (2014).
\newblock It\^{o} and Stratonovich Stochastic Calculus with \R.
\newblock In preparation (Computational Statistics, Springer).
...
\bibitem[Daniel and Duncan, 2014]{rgl}
Daniel, A. and  Duncan, M. (2014).
\newblock \CRANpkg{rgl}: 3D visualization device system (OpenGL).
\newblock \emph{\R~package version 0.93.996}.
\newblock \url{http://CRAN.R-project.org/package=rgl}

\bibitem[Uwe et all, 2014]{scatterplot3d}
Uwe, L., Martin, M. and Sarah, S. (2014).
\newblock \CRANpkg{scatterplot3d}: 3D Scatter Plot.
\newblock \emph{\R~package version 0.3-35}.
