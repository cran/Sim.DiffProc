...
\title{\bfseries{It\^{o} and Stratonovich Stochastic Calculus with}\\
      \vskip3pt \large \CRANpkg{Sim.DiffProc} Package Version 3.3}
...
\begin{abstract}
This paper describes a detailed tutorial for the \pkg{Sim.DiffProc} package for numerically computing solutions for stochastic differential equations (SDE's). We develop three core functions: \code{snssde1d}, \code{snssde2d}, and \code{snssde3d}. These functions are used for simulating trajectories of flows for uni- and multidimensional SDE's with different approximation schemes, in the case of independent standard Brownian motions. In addition, basic statistics of the solution of SDE's are provided using the Monte-Carlo method. The package implementions are illustrated with many examples. Furthermore, we study the special case of a uni-dimensional SDE driven by three independent standard Brownian motions. The solution of which is obtained through a transformation to a system of three equations.
\end{abstract}
...
\begin{itemize}
  ...
  \item[$\circ$] Monte-Carlo statistics of the solution process at time $t$, i.e. mean, median, quantiles, moments, skewness, kurtosis, $\alpha \%$ confidence bands,$\dots$.
\end{itemize}
...
simulations, the following statistical measures (\code{S3 method} for class \code{'snssde1d'}) can be approximated for the $X_{t}$ process at time $t$, i.e. $X_{t}$:
\begin{enumerate}
  \item the mean value $\E(X_{t})$; using the command \code{mean}.
