...
.Euler1D <- function(N =100,M=1,x0=2,t0=0,T=1,Dt,drift,diffusion,
                          type=c("ito","str"),...)
                       {  
    if (missing(Dt)) {
    ...
    X[1] <- x0 
    for (i in 2:(N+1)){
      X[i] = X[i-1] + A(t[i-1],X[i-1])*Dt + S(t[i-1],X[i-1])*dw[i-1]
       }
    X
         }
    res <- data.frame(replicate(M,euler1d()))
...
.Euler2D <- function(N =100,x0=2,y0=1,t0=0,T=1,Dt,driftx,diffx,drifty,diffy,
                          type=c("ito","str"),...)
                       {   
    if (missing(Dt)) {
        t <- seq(t0, T, length = N + 1)
    ...
    Y[1] <- y0
    for (i in 2:(N+1)){
      X[i] = X[i-1] + Ax(t[i-1],X[i-1],Y[i-1])*Dt + Sx(t[i-1],X[i-1],Y[i-1])*dwx[i-1]
      Y[i] = Y[i-1] + Ay(t[i-1],X[i-1],Y[i-1])*Dt + Sy(t[i-1],X[i-1],Y[i-1])*dwy[i-1]
       }
    res <- data.frame(X,Y)
...
.Euler3D <- function(N =100,x0=2,y0=1,z0=1,t0=0,T=1,Dt,driftx,diffx,drifty,diffy,
    ...
    Z[1] <- z0
    for (i in 2:(N+1)){
      X[i] = X[i-1] + Ax(t[i-1],X[i-1],Y[i-1],Z[i-1])*Dt + Sx(t[i-1],X[i-1],Y[i-1],Z[i-1])*dwx[i-1]
      Y[i] = Y[i-1] + Ay(t[i-1],X[i-1],Y[i-1],Z[i-1])*Dt + Sy(t[i-1],X[i-1],Y[i-1],Z[i-1])*dwy[i-1]
      Z[i] = Z[i-1] + Az(t[i-1],X[i-1],Y[i-1],Z[i-1])*Dt + Sz(t[i-1],X[i-1],Y[i-1],Z[i-1])*dwz[i-1]
       }
